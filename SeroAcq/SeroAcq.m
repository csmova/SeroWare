...
% Last Modified by GUIDE v2.5 20-Apr-2023 13:48:14
...
global txtRV
global daqName
global specMeasCycles discardData VoltageDriver myCustomColormap cpRefreshToDisplay cpBGSinterval
...
myCustomColormap = [0,0,1;
    0.00871459767222405,0.0187363848090172,0.949891090393066;
    0.0174291953444481,0.0374727696180344,0.899782121181488;
    0.0261437911540270,0.0562091507017612,0.849673211574554;
    0.0348583906888962,0.0749455392360687,0.799564301967621;
    0.0435729846358299,0.0936819165945053,0.749455332756043;
    0.0522875823080540,0.112418301403522,0.699346423149109;
    0.0610021799802780,0.131154686212540,0.649237513542175;
    0.0697167813777924,0.149891078472137,0.599128544330597;
    0.0784313753247261,0.168627455830574,0.549019634723663;
    0.0686274543404579,0.147549018263817,0.480392187833786;
    0.0588235333561897,0.126470595598221,0.411764740943909;
    0.0490196086466312,0.105392158031464,0.343137264251709;
    0.0392156876623631,0.0843137279152870,0.274509817361832;
    0.0294117666780949,0.0632352977991104,0.205882370471954;
    0.0196078438311815,0.0421568639576435,0.137254908680916;
    0.00980392191559076,0.0210784319788218,0.0686274543404579;
    0,0,0;0.200000002980232,0.200000002980232,0;
    0.400000005960465,0.400000005960465,0;
    0.600000023841858,0.600000023841858,0;
    0.800000011920929,0.800000011920929,0;
    1,1,0;0.985098063945770,0.945588231086731,0.00153594766743481;
    0.970196068286896,0.891176462173462,0.00307189533486962;
    0.955294132232666,0.836764693260193,0.00460784323513508;
    0.940392136573792,0.782352924346924,0.00614379066973925;
    0.925490200519562,0.727941155433655,0.00767973857000470;
    0.910588264465332,0.673529386520386,0.00921568647027016;
    0.895686268806458,0.619117617607117,0.0107516339048743;
    0.880784332752228,0.564705848693848,0.0122875813394785;
    0.865882337093353,0.510294079780579,0.0138235297054052;
    0.850980401039124,0.455882340669632,0.0153594771400094;
    0.815032720565796,0.392973840236664,0.0435185208916664;
    0.779084980487824,0.330065339803696,0.0716775581240654;
    0.743137240409851,0.267156869173050,0.0998366028070450;
    0.707189559936523,0.204248368740082,0.127995640039444;
    0.671241879463196,0.141339868307114,0.156154692173004;
    0.635294139385223,0.0784313753247261,0.184313729405403;
    0.611764729022980,0.0960784330964088,0.246405243873596;
    0.588235318660736,0.113725490868092,0.308496743440628;
    0.564705908298492,0.131372556090355,0.370588243007660;
    0.541176497936249,0.149019613862038,0.432679772377014;
    0.517647087574005,0.166666671633720,0.494771271944046;
    0.494117647409439,0.184313729405403,0.556862771511078;
    0.329411774873734,0.456209152936935,0.704575181007385;
    0.164705887436867,0.728104591369629,0.852287590503693;
    0,1,1;0,0.928291320800781,0.857142865657806;
    0,0.856582641601563,0.714285731315613;
    0,0.784873962402344,0.571428596973419;
    0,0.713165283203125,0.428571432828903;
    0,0.641456604003906,0.285714298486710;
    0,0.569747924804688,0.142857149243355;
    0,0.498039215803146,0;
    0,0.553812623023987,0;
    0,0.609586060047150,0;
    0,0.665359497070313,0;
    0,0.721132874488831,0;
    0,0.776906311511993,0;
    0,0.832679748535156,0;
    0,0.888453185558319,0;
    0,0.944226562976837,0;
    0,1,0];
...
VoltageDriver = 1.0; %initialize as working electrode driven

cpRefreshToDisplay=5; %initialize colorplot settings
cpBGSinterval=5;
...
global daqName VoltageMultiplier WaveformName SampledPoints StimLabelOnly linehAppliedSignal phAppliedSignal
...
if NewFileName~=0
    ...
    try load(strcat(PathName,FileName),'-mat','WaveformName')
    ...
    %pineMultiplier = str2double(inputdlg('Set potentiostat multiplier value if desired (e.g., 3). 
    %If you accounted for the multiplier during signal generation, insert 1.','Set Multiplier Value'));
    VoltageMultiplier = str2double(handles.txtMultiplier.String);
    handles.lblMultiplier.String = VoltageMultiplier;  
    if isnan(VoltageMultiplier)
        errordlg('Voltage multiplier must be a number.');
        return
    end
...
%     ylabel(handles.plotCV,'Current (nA)');
    ...
    cla(handles.pltAppliedSignal);
    %plot waveform
    linehSignal=line(handles.pltCycle,(1:PointsPerCycle),(Signal_Cycle(1:PointsPerCycle))/VoltageMultiplier,'Color','b');
    linehAppliedSignal=line(handles.pltAppliedSignal,(1:TotalPoints),(Signal_Cycle(1:TotalPoints))/VoltageMultiplier);
    phAppliedSignal=handles.pltAppliedSignal;
    %drawnow;
    %pause(1);
...
global specMeasCycles linehStim
...
if strcmp(Answer,'Yes')==1
    ...
    if strcmp(Answer,'Yes')==1
        ...
        if FileName~=0
            ...
            handles.optWEdriven.Enable='off'
            handles.optREdriven.Enable='off'
            handles.opt200.Enable='off';
            handles.opt1000.Enable='off';
            handles.optCustum.Enable='off';
            handles.txtCustomGain.Enable='off';
            handles.txtMultiplier.Enable='off';
            ...
            linehStim=line(handles.pltStim,0,0);
            ...
            ylabel(handles.pltColorPlot,'Voltage');
...
global chkRV 
global txtRV linehAppliedSignal Signal_Cycle VoltageMultiplier phAppliedSignal
...
if Accumulation_Running==1
    AppliedSignal(1:TotalPoints*CyclesPerRefresh)=AccuVoltage*VoltageMultiplier; %set signal to accumulation voltage
    ...
    if AccuRefresh==TotalAccuRefreshCycles %note that a refresh cycle is a multiple of the waveform cycle (ex: 5)
     ...
     if chkRV==1
        RestVoltage=txtRV/1000*VoltageMultiplier; %convert mV to V
        AppliedSignal = []; 
...
cla(phAppliedSignal);
delete(linehAppliedSignal)
linehAppliedSignal=line(phAppliedSignal,(1:TotalPoints),(AppliedSignal(1:TotalPoints))/VoltageMultiplier);
xlabel(phAppliedSignal,'Total Points');
ylabel(phAppliedSignal,'Voltage (V)');
title(phAppliedSignal,'Applied Waveform');
...
if s.IsRunning==true
    h=msgbox('Stopping session. Do not press any buttons.','In progress','warn');
    stop(s);
end
...
h=msgbox('Saving data. Do not press any buttons.','In progress','warn');
...
if (IgnoreSelected == 1) %status of ignore checkbox when data collection was started
    ...
    save(strcat(C.PathDataFiles,FileName),'N_Cycles','Saved_Data','Execution_Time','Injections','N_Injections','InjectionLabel','Gain','PointsPerCycle','Signal_Cycle','OxPeakVM','N_Peaks','SP_Peaks','PP_Peaks','EP_Peaks','PeaksLabel', 'IgnoreSelected','CP_Data_All','SignalFrequency', 'VoltageMultiplier', 'ScanRate', 'WaveformName');
delete(h);
h=msgbox('Data saved!','Success','help');
delete(h);
end
...
global CV_Data Signal_Cycle phCV discardData phCP CP_Data CP_Data_All generateCP myCustomColormap cpRefreshToDisplay chkCPbgs chkCPenable cpBGSinterval
...
CP_Data = zeros(PointsPerCycle, CyclesPerRefresh*RefreshPerWindow); %create matrix for color plot current intensity
CP_BGS_Data=[];
...
%realtime CP code
if chkCPenable==1 
    xlabel(phCP,'Time (cycles)');
    ylabel(phCP,'Voltage');
    if chkCPbgs == 0 %bgs stands for background subtraction
        if N_Refresh>cpRefreshToDisplay
            cla(phCP); % clears handle for Color Plot (CP)
            %disp(size(Total_Data(((N_Refresh-cpRefreshToDisplay)*CyclesPerRefresh*PointsPerCycle+1):((N_Refresh)*CyclesPerRefresh*PointsPerCycle))))
            %disp(PointsPerCycle)
            %disp(CyclesPerRefresh*cpRefreshToDisplay)
            CP_Data = reshape(Total_Data(((N_Refresh-cpRefreshToDisplay)*CyclesPerRefresh*PointsPerCycle+1):((N_Refresh)*CyclesPerRefresh*PointsPerCycle)),PointsPerCycle,CyclesPerRefresh*cpRefreshToDisplay);
            colorplot = pcolor(phCP,1:CyclesPerRefresh*cpRefreshToDisplay,1:PointsPerCycle,Gain*CP_Data);
            colorplot.EdgeColor = 'none';
            colormap(phCP,myCustomColormap); 
            c=colorbar(phCP);
            xlabel(phCP,'Time (cycles)');
            ylabel(phCP,'Sampled Point');
            c.Label.String = 'Current (nA)';
        else %dont plot anything if not enough refreshes have occured  
            CP_Data = [];
            xlabel(phCP,'Time (cycles)');
            ylabel(phCP,'Sampled Point');
        end
    ...
    if chkCPbgs == 1
        if N_Refresh>cpRefreshToDisplay
            if N_Refresh>cpBGSinterval %if time for BGS is > BGS interval (ie 30 s)
                cla(phCP); % clears handle for Color Plot (CP)
                CP_Data = reshape(Total_Data(((N_Refresh-cpRefreshToDisplay)*CyclesPerRefresh*PointsPerCycle+1):((N_Refresh)*CyclesPerRefresh*PointsPerCycle)),PointsPerCycle,CyclesPerRefresh*cpRefreshToDisplay);
                CP_BGS_Data = reshape(Total_Data(((N_Refresh-cpRefreshToDisplay-cpBGSinterval)*CyclesPerRefresh*PointsPerCycle+1):((N_Refresh-cpRefreshToDisplay)*CyclesPerRefresh*PointsPerCycle)),PointsPerCycle,CyclesPerRefresh*cpBGSinterval);
                CP_BGS_Data = mean(CP_BGS_Data,2);
                CP_Data = bsxfun(@minus, CP_Data, CP_BGS_Data);
                colorplot = pcolor(phCP,1:CyclesPerRefresh*cpRefreshToDisplay,1:PointsPerCycle,Gain*CP_Data);
                colorplot.EdgeColor = 'none';
                colormap(phCP,myCustomColormap);%TODO:allow loading other custom colormap vars
                c=colorbar(phCP); 
                c.Label.String = 'Current (nA)';
                xlabel(phCP,'Time (cycles)');
                ylabel(phCP,'Sampled Point');
...
hcmdStop.Enable='off';
hcmdResetPlot.Enable='off';
handles.optWEdriven.Enable='on';
handles.optREdriven.Enable='on';
handles.opt200.Enable='on';
handles.opt1000.Enable='on';
handles.optCustum.Enable='on';
handles.txtCustomGain.Enable='on';
handles.txtMultiplier.Enable='on';
hcmdInjection.Enable='off';
cmdhStimulation.Enable='off';
opthInjectionMode.Enable='on';
opthStimulationMode.Enable='on';
...
global AccumulationFlag TotalAccuRefreshCycles AccuRefresh AccuVoltage StimLabelOnly linehStim SignalFrequency SignalPeriod
...
if StimLabelOnly == 0 %if we only want to do an event marker, do not make a stim signal
    ...
    StimPulseWidth=str2double(handles.txtStimPulseWidth.String)/1000; %converts ms to s
    ...
    if handles.chkStimNoiseReduction.Value==0 %no noise reduction
        Cycle=round(ScanRate/StimFrequency); %what fraction of time is spent stimulating (points per stim)
        if handles.chkBiphasicStimulation.Value==0 %not biphasic
            for i=1:StimFrequency*StimDuration %number of stims
                for j=1:StimPulseWidth*ScanRate %number of points/pulse
        ...
        NPulsesPerCycle=StimFrequency/SignalFrequency;%TODO: correct? %TODO: handle non-integer? =stims/wf?
        ...
        IntervalPulses=(SignalPeriod-PointsPerCycle/ScanRate+0.001)/(StimFrequency*SignalPeriod);
        %TODO:correct?
        StimOffset=(PointsPerCycle/ScanRate+0.001); %TODO: allow custom offset?
        if handles.chkBiphasicStimulation.Value==0 %not biphasic
            for i=1:StimDuration*SignalFrequency %TODO: update? why 10? why 0.1?... for each stim; TODO: SignalFreq or StimFreq?
                for j=1:NPulsesPerCycle %...for each pulse in each stim
                    for k=1:StimPulseWidth*ScanRate %...for each point in each pulse in each stim
                        StimSignal(round((i-1)*SignalPeriod*ScanRate+(StimOffset+(j-1)*IntervalPulses)*ScanRate+k))=StimVoltage; %todo: correct?
                    end
                end
            end
        else %biphasic
            for i=1:StimDuration*SignalFrequency %TODO: update? why 10? =SigFreq?why 0.1?=SigPer?
                for j=1:NPulsesPerCycle
                    for k=1:round(StimPulseWidth*ScanRate/2) %first half of pulse
                        StimSignal(round((i-1)*SignalPeriod*ScanRate+(StimOffset+(j-1)*IntervalPulses)*ScanRate+k))=StimVoltage;
                    end
...
linehStim=line(handles.pltStim,1:TotalPoints,StimSignal(1:TotalPoints));
xlabel(handles.pltStim,'Total Points');
ylabel(handles.pltStim,'Voltage (V)');
title(handles.pltStim,'Stimulation Waveform');
StimFlag=1;
StimRefresh=0;
...
function txtCPRefreshToDisplay_Callback(hObject, eventdata, handles)
% hObject    handle to txtCPRefreshToDisplay (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hints: get(hObject,'String') returns contents of txtCPRefreshToDisplay as text
%        str2double(get(hObject,'String')) returns contents of txtCPRefreshToDisplay as a double
global cpRefreshToDisplay

cpRefreshToDisplay = str2double(get(hObject,'String'));

% --- Executes during object creation, after setting all properties.
function txtCPRefreshToDisplay_CreateFcn(hObject, eventdata, handles)
% hObject    handle to txtCPRefreshToDisplay (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    empty - handles not created until after all CreateFcns called
...
% --- Executes on button press in chkCPbgs.
function chkCPbgs_Callback(hObject, eventdata, handles)
% hObject    handle to chkCPbgs (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hint: get(hObject,'Value') returns toggle state of chkCPbgs
global chkCPbgs
chkCPbgs=get(hObject,'Value');


function txtBGSinterval_Callback(hObject, eventdata, handles)
% hObject    handle to txtBGSinterval (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hints: get(hObject,'String') returns contents of txtBGSinterval as text
%        str2double(get(hObject,'String')) returns contents of txtBGSinterval as a double
global cpBGSinterval
 %TODO:convert to s
cpBGSinterval = get(hObject,'String');

% --- Executes during object creation, after setting all properties.
function txtBGSinterval_CreateFcn(hObject, eventdata, handles)
% hObject    handle to txtBGSinterval (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    empty - handles not created until after all CreateFcns called
...
% --- Executes on button press in chkCPenable.
function chkCPenable_Callback(hObject, eventdata, handles)
% hObject    handle to chkCPenable (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hint: get(hObject,'Value') returns toggle state of chkCPenable
global chkCPenable

chkCPenable=get(hObject,'Value');
